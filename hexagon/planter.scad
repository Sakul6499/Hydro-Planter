      ...
      difference() {
        union() {
          ...
          color("lime") difference() {
            cylinder(h = $height - $connection, r = $radius, $fn = 6,
                     center = true);

            // DIFF
            translate([ 0, 0, $margin ]) cylinder(
                h = $height, r = $radius - $margin, center = true, $fn = 6);
          }
          ...
          color("tomato") translate([ 0, 0, -($height / 2) ]) {
            union() {
              // Connector part
              cylinder(h = $connection, r = $radius - $margin, $fn = 6,
                       center = true);

              // Connecting bit to outer shell
              translate([ 0, 0, $connection ]) cylinder(
                  h = $connection, r = $radius - .1, $fn = 6, center = true);
            }
          }
        }
        // DIFF
